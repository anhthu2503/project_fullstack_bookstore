...
const Results = ({ className, donhangs, ...rest }) => {
  ...
  const handleSelectAll = (event) => {
    ...
    if (event.target.checked) {
      newSelectedDonHangIds = donhangs.map((donhang) => donhang.ID);
    } else {
            ...
            <TableBody>
              {donhangs.slice(0, limit).map((donhang) => (
                <TableRow
                  hover
                  key={donhang.ID}
                  selected={selectedDonHangIds.indexOf(donhang.ID) !== -1}
                >
                  <TableCell padding="checkbox">
                    <Checkbox
                      checked={selectedDonHangIds.indexOf(donhang.ID) !== -1}
                      onChange={(event) => handleSelectOne(event, donhang.ID)}
                      value="true"
                    />
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <div>
                      Họ tên:
                      {donhang.Fullname_nguoi_nhan}
                    </div>
                    <div>
                      Số điện thoại:
                      {donhang.Sđt_nguoi_nhan}
                    </div>
                    <div>
                      Địa chỉ:
                      {donhang.Address_nguoi_nhan}
                    </div>
                    <div>
                      Email:
                  ...
                  <TableCell>
                    {donhang.list_san_pham.map((sanpham) => {
                      return (<div>{`${sanpham.Ten}-${sanpham.So_luong}-${sanpham.Don_gia}-${sanpham.Thanh_tien}`}</div>);
                    })}
