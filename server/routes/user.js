...
router.delete('/:id_user', authenticate.auth, (req, res) => {
    ...
    MongoClient.connect(url, function(err, client) {
        ...
        collection_user.deleteOne({_id: ObjectID(req.params.id_user)}, () => {
            res.json({
                'xu_ly': 'delete user ' + req.params.id_user + ' thành công',
                data_send: req.body
            });
        });
    });
            ...
            if(results[0].mat_khau == md5(req.body.mat_khau)){
                ...
                connection.query(`SELECT * FROM token WHERE user_id = ?`, [results[0].id], function (error, results_token, fields){
                    ...
                    if(results_token.length){//nếu có token rồi
                        ...
                        expireted_date = ?
                        WHERE user_id = ?`,
                        [string_token, expireted_date, results[0].id],
                        ...
                        connection.query(`INSERT INTO token(token, created_date, expireted_date, user_id, type_token)
                                            VALUES(?, ?, ?, ?, ?)`,
                        [string_token, created_date, expireted_date, results[0].id, 'authorized'],
                        function (error, results, fields){
                            if (error) throw error;
